...
### Managing AuthKit Application Settings
...
The AuthKit homepage URL is not exposed as a Terraform resource because the provider
needs stable read semantics to support drift detection.
...
Webhook signing secrets are generated by WorkOS and exposed as a computed,
sensitive `secret` attribute when the API returns them. Store Terraform state in a
secure backend and do not commit state or plan files.
...
terraform import workos_redirect_uri.production_callback redirect_uri_01HXYZ...
terraform import workos_cors_origin.production_web cors_origin_01HXYZ...
terraform import workos_webhook_endpoint.app webhook_endpoint_01HXYZ...
```
...
| `workos_redirect_uri` | Manages AuthKit redirect URIs |
| `workos_cors_origin` | Manages AuthKit CORS origins |
| `workos_webhook_endpoint` | Manages WorkOS webhook endpoints |
